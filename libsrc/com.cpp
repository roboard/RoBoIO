...
RBAPI(bool) com_Init(int com, int duplex) {
    #define COM_NAME_MAPPING(x)     (x+1)

    if (com_InUse(com) == true)
	{
        err_SetMsg(ERROR_COM_INUSE, "COM%d was already opened", COM_NAME_MAPPING(com));
		return false;
	}
    ...
    {
        err_SetMsg(ERROR_COM_INVALID, "COM%d isn't enabled in BIOS", COM_NAME_MAPPING(com));
        goto COMINIT_FAIL;
    }
    ...
    COM_oldFMode[com] = com_IsFIFO32Mode(com);
    	...
    	{
            err_SetMsg(ERROR_COM_FAIL, "cannot open COM%d device driver", COM_NAME_MAPPING(com));
            goto COMINIT_FAIL;
    	}
    ...
    /* NOTE: Vortex86DX's UART 32-byte FIFO mode has a bug and cannot work normally
