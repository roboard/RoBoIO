...
static unsigned long RCSERVO_selChannels   = RCSERVO_USENOPIN;  // for rcservo_SetServo(...)
static unsigned long RCSERVO_usedChannels  = RCSERVO_USENOPIN;
static unsigned long RCSERVO_validChannels = RCSERVO_USENOPIN;  // used in Play Mode
...
RBAPI(bool) rcservo_InitP(void) {
    return rcservo_Init(RCSERVO_selChannels);
}

RBAPI(bool) rcservo_Init(unsigned long usedchannels) {        // for compatibility to RoBoIO 1.81
...
RBAPI(void) rcservo_Close(void) {
    ...
    RCSERVO_selChannels   = RCSERVO_USENOPIN;
    RCSERVO_usedChannels  = RCSERVO_USENOPIN;
    RCSERVO_validChannels = RCSERVO_USENOPIN;
	RCSERVO_inUse = false;
}
...
CAPTURE_END:
    ...
    if (width == 0xfffffffeL)
    {
        err_SetMsg(ERROR_RCSERVO_CAPFAIL, "wait no the raising edge of the feedback pulse");
        return 0xffffffffL;
    }
    else
    if (width == 0xfffffffdL)
    {
        err_SetMsg(ERROR_RCSERVO_CAPFAIL, "wait no the falling edge of the feedback pulse");
        return 0xffffffffL;
    }
	return width;
...
RBAPI(unsigned long) rcservo_ReadPosition(int channel, int cmd) {
	...
	if (RCSERVO_operationMode != RCSERVO_CAPTUREMODE)
	{
        ...
        err_SetMsg(ERROR_RCSERVO_CAPFAIL, "%s() can't work in PLAY or PWM mode\n", __FUNCTION__);
		return 0xffffffffL;
	}
	...
	if ((cmd < 0) || (cmd > 7))
	{
        ...
        err_SetMsg(ERROR_RCSERVO_CAPFAIL, "%s() receives an unknown command for reading feedback\n", __FUNCTION__);
		return 0xffffffffL;
	}
...
RBAPI(void) rcservo_ReadPositions(unsigned long channels, int cmd, unsigned long* width) {
	...
	for (i=0; i<32; i++)
	{
        ...
        if ((width[i] = rcservo_ReadPosition(i, cmd)) == 0xffffffffL)
        {
            caperr = true;
            strcpy(errbuf, roboio_GetErrMsg());
        }
	}
	MPOS_Ends();

    if (caperr == true) err_SetMsg(ERROR_RCSERVO_CAPFAIL, errbuf);
}
...
RBAPI(void) rcservo_ReadPositionsDN(unsigned long channels, int cmd, unsigned long* width) {
	...
	for (i=0; i<32; i++)
	{
        ...
        if ((width[i] = rcservo_ReadPositionDN(i, cmd)) == 0xffffffffL)
        {
            caperr = true;
            strcpy(errbuf, roboio_GetErrMsg());
        }
...
static unsigned long linear_interpolation(unsigned long start_val, unsigned long end_val, unsigned long total, unsigned long t) {
	if (total == 0L) return end_val;

	return ((total-t)*start_val + t*end_val)/total;
}
static unsigned long sine_interpolation(unsigned long start_val, unsigned long end_val, unsigned long total, unsigned long t) {
    #define HALF_PI (3.14159265/2.0)
	double r;

	if (total == 0L) return end_val;

	r = sin((double)t*HALF_PI/(double)total);
	return (unsigned long)((1.0-r)*(double)start_val + r*(double)end_val);
}

_RB_INLINE void read_positions_playmode(unsigned long* width) { //used by rcservo_EnterPlayMode()
	int i;
...
RBAPI(void) rcservo_EnterPlayMode(void) {
	...
	read_positions_playmode(RCSERVO_curaction);  // will initialize RCSERVO_validChannels
	for (i=0; i<32; i++)
    {
        ...
        if (RCSERVO_svParams[i].servoType != RCSERVO_SV_FEEDBACK)
        {
            ...
            if (RCSERVO_svParams[i].servoType == RCSERVO_SV_NOFEEDBACK)
                errmsg("WARNING: %s() doesn't support channel %d of NOFEEDBACK-type!\n", __FUNCTION__, i);
        }
...
static int play_action(unsigned long (*ifunc)(unsigned long, unsigned long, unsigned long, unsigned long)) {
    unsigned long t, tt;
    int i;
    ...
    if (t >= (RCSERVO_curaction_nexttime - RCSERVO_curaction_starttime))
    {
        ...
        else
        {
            ...
            {
                if ((RCSERVO_validChannels & (1L<<i)) == 0L) continue;
                RCSERVO_midaction[i] = ifunc(RCSERVO_lastaction[i], RCSERVO_curaction[i], tt, t);
            }
        }
...
static int play_action_mix(long* mixwidth, unsigned long (*ifunc)(unsigned long, unsigned long, unsigned long, unsigned long)) {
...
RBAPI(int) rcservo_PlayAction(void) {
    return play_action(linear_interpolation);
}
RBAPI(int) rcservo_PlayActionS(void) {
    return play_action(sine_interpolation);
}

RBAPI(int) rcservo_PlayActionMix(long* mixwidth) {
    return play_action_mix(mixwidth, linear_interpolation);
}
...
RBAPI(bool) rcservo_PinMode(int channel, unsigned pinmode) {
	...
	if (pinmode == RCSERVO_GPIO)
	{
        RCSERVO_selChannels = RCSERVO_selChannels & ~LONG_LSHIFT(1L, channel);
        return true;
	}

	if (rcservo_SetServo(channel, pinmode) == true)
	{
        RCSERVO_selChannels = RCSERVO_selChannels | LONG_LSHIFT(1L, channel);
        return true;
	}
...
*/
		...
		case RCSERVO_TOWERPRO_SG90:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 6000L, 560L, 2640L);
			break;

		case RCSERVO_TOWERPRO_SG5010:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 6000L, 510L, 2420L);
			break;

		case RCSERVO_SCANNER_STL9894CTG:

            rcservo_SetServo(channel, RCSERVO_SERVO_DEFAULT_NOFB);
            rcservo_SetServoParams1(channel, 3000L, 780L, 2170L);
			break;
