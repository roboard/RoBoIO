...
RBAPI(bool) rcservo_InitP(void);  // init all PWM/GPIO pins according to their modes
...
//-- values for the "pinmode" argument
     #define RCSERVO_GPIO                    (0xffff)   // set the pin as GPIO mode
     #define RCSERVO_SERVO_DEFAULT           (0x00)     // set the pin as SERVO mode; conservative setting for most servos with pulse feedback
     #define RCSERVO_SERVO_DEFAULT_NOFB      (0x01)     // set the pin as SERVO mode; conservative setting for most servos without pulse feedback
...
//   #define RCSERVO_KONDO_KRS4034           (0x16)     // not directly work on RB-100/RB-100RD/RB-110/RB-050 due to the unmatched PWM pull-up/-down resistors
     ...
     #define RCSERVO_TOWERPRO_SG90           (0x53)
     #define RCSERVO_TOWERPRO_SG5010         (0x54)
     ...
     #define RCSERVO_SCANNER_STL9894CTG      (0x81)
...
//   #define RCSERVO_PINS1                   (0)
//   #define RCSERVO_PINS2                   (1)
//   ...... (refer to the "channel" argument of rcservo_PinMode())
...
RBAPI(int)  rcservo_PlayAction(void);   // move servos using linear interpolation
RBAPI(int)  rcservo_PlayActionS(void);  // move servos using sine interpolation
...
//***********  Constants for Compatibility to RoBoIO 1.81  ***********
...
RBAPI(bool) rcservo_SetServo(int channel, unsigned servono);                // for compatibility to RoBoIO 1.81
RBAPI(bool) rcservo_SetServos(unsigned long channels, unsigned servono);    // for compatibility to RoBoIO 1.81

RBAPI(unsigned long) rcservo_ReadPosition(int channel, int cmd);  // for compatibility to RoBoIO 1.81
//-- values for the "channel" argument
//   #define RCSERVO_PINS1                   (0)
//   #define RCSERVO_PINS2                   (1)
//   ...... (refer to the "channel" argument of rcservo_PinMode())
//-- if the above functions return false, roboio_GetErrCode() may return:
     #define ERROR_RCSERVO_CAPFAIL           (ERR_NOERROR + 471)
RBAPI(void) rcservo_ReadPositions(unsigned long channels, int cmd, unsigned long* width);  // for compatibility to RoBoIO 1.81
//-- values for the "channels" argument (note that different values can be added to use multiple channels)
//   #define RCSERVO_USEPINS1                (1L)
//   #define RCSERVO_USEPINS2                (1L<<1)
//   ...... (refer to the "usedchannels" argument of rcservo_Init())
...
//-- if fails to capture some channels, roboio_GetErrCode() may return for the last failed channel:
//   #define ERROR_RCSERVO_CAPFAIL           (ERR_NOERROR + 471)
